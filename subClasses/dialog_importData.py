...
class Ui_ImportData(object):
    def setupUi(self, ImportData):
        ...
        self.tableWidget__dfPreview.setGeometry(QtCore.QRect(390, 30, 361, 61))
        ...
        self.DataIsAlreadyCopied = False
        ...
        # Buttons connexion
        self.toolButton_OpenFile.clicked.connect(self.open_file_dialog)
        ...
        self.comboBox_typeAsset.currentTextChanged.connect(self.onSelectAssetType)
        ...
        with open(self.path_classification_data_json,'r') as f:
            ...
            f.close()
        list_type_assets = [key for key in dic_classification_imported_data.keys()]
        # add row to asset type combobox
        for i in range(len(list_type_assets)):
            assetType_name = list_type_assets[i]
            self.comboBox_typeAsset.addItem("")
            self.comboBox_typeAsset.setItemText(i, self._translate("ImportData", assetType_name))
        self.comboBox_typeAsset.setCurrentText(list_type_assets[0])
        current_selected_assetType = self.comboBox_typeAsset.currentText()
        list_current_broker = dic_classification_imported_data[current_selected_assetType]
        for i in range(len(list_current_broker)):
            broker_name = list_current_broker[i]
            self.comboBox_broker.addItem("")
            self.comboBox_broker.setItemText(i, self._translate("ImportData", broker_name))

        

    def retranslateUi(self, ImportData):
        self._translate = QtCore.QCoreApplication.translate
        ImportData.setWindowTitle(self._translate("ImportData", "Dialog"))
        self.comboBox_DataType.setItemText(0, self._translate("ImportData", "tick data"))
        self.comboBox_DataType.setItemText(1, self._translate("ImportData", "fixed data"))
        self.label.setText(self._translate("ImportData", "Select the data to import: "))
        self.toolButton_OpenFile.setText(self._translate("ImportData", "..."))
        self.label_2.setText(self._translate("ImportData", "Select type of data:"))
        self.label_3.setText(self._translate("ImportData", "Select name of date column:"))
        self.label_4.setText(self._translate("ImportData", "Select name of time column:"))
        self.label_6.setText(self._translate("ImportData", "Select date format:"))
        self.comboBox_nameTimestamp.setItemText(0, self._translate("ImportData", "create new (input right)"))
        self.label_datefromatinfo.setText(self._translate("ImportData", "(1st date is 2020-08-01 01:00:02)"))
        self.groupBox_selectclassification.setTitle(self._translate("ImportData", "Select Classification"))
        self.label_8.setText(self._translate("ImportData", "Select Broker:"))
        self.label_5.setText(self._translate("ImportData", "Select Type of Asset:"))
        self.groupBox_addNewClassification.setTitle(self._translate("ImportData", "Add new Classifications"))
        self.label_9.setText(self._translate("ImportData", "Name new Broker:"))
        self.label_7.setText(self._translate("ImportData", "Name new Asset Type:"))
        self.pushButton_submitAssetType.setText(self._translate("ImportData", "Submit"))
        self.pushButton_submitnewBroker.setText(self._translate("ImportData", "Submit"))
        self.pushButton_show1strow.setText(self._translate("ImportData", "Show 1st row"))
    ...
    def submit_importedData(self):
        ...
        if not self.DataIsAlreadyCopied:
            shutil.copy(path_src, path_target)
            print('we copy with shutil')
    ...
    def open_file_dialog(self):
        ...
        self.namefile_imported_data = self.path_src.split('/')[-1]
        ...
        if not self.data_have_been_imported:
            ...
            self.DataIsAlreadyCopied = True
        list_columns = [col for col in self.df.columns]
        for i in range(len(list_columns)):
            col_name = list_columns[i]
            self.comboBox_nameDateColumn.addItem("")
            self.comboBox_nameDateColumn.setItemText(i, self._translate("ImportData", col_name))
            self.comboBox_nameTimestamp.addItem("")
            self.comboBox_nameTimestamp.setItemText(i+1, self._translate("ImportData", col_name))
            if i == 0:
                self.comboBox_nameDateColumn.setCurrentText(col_name)
        self.comboBox_nameTimestamp.setCurrentText("create new (input right)")
    ...
    def function_showPreview(self):
        ...
        self.tableWidget__dfPreview.setRowCount(1)
        self.tableWidget__dfPreview.setHorizontalHeaderLabels(list_columns_str)
        for i in range(0,len(self.df.columns)):
            self.tableWidget__dfPreview.setItem(0,i, QtWidgets.QTableWidgetItem(list_values_str[i]))
    ...
    def onSelectAssetType(self):
        '''Change the choices of combo box broker according to the ones available for the asset type'''
        # extract asset type
        current_assetType = self.comboBox_typeAsset.currentText()
        if current_assetType != '':
            # extract broker list
            with open(self.path_classification_data_json,'r') as f:
                dic_imported_data_sumup = json.load(f)
                f.close()
            list_available_broker = dic_imported_data_sumup[current_assetType]
            # create new broker combo box
    ...
    def function_submitNewAssetType(self):
        ...
        # extract name
        new_assetType_name = self.textEdit_assetType.toPlainText()
        # put it in json classification
        with open(self.path_classification_data_json,'r') as f:
            dic_classification_imported_data = json.load(f)
            f.close()
        dic_classification_imported_data[new_assetType_name] = []
        with open(self.path_classification_data_json,'w') as f:
            json.dump(dic_classification_imported_data, f)
            f.close()  
        # add row to asset type combobox
        self.comboBox_typeAsset.addItem("")
        self.comboBox_typeAsset.setItemText(len(dic_classification_imported_data)-1, self._translate("ImportData", new_assetType_name))
    ...
    def function_submitNewBroker(self):
        ...
        with open(self.path_classification_data_json,'r') as f:
            dic_classification_imported_data = json.load(f)
            f.close()
        dic_classification_imported_data[current_assetType].append(new_broker_name)
        with open(self.path_classification_data_json,'w') as f:
            json.dump(dic_classification_imported_data, f)
            f.close()  
        # add row to broker combobox
        self.comboBox_broker.addItem("")
        self.comboBox_broker.setItemText(len(dic_classification_imported_data[current_assetType])-1, self._translate("ImportData", new_broker_name))
    ...
    def create_dic_imported_data(self):
        ...
        if type(self.df[name_date_column].iloc[0]) == int or type(self.df[name_date_column].iloc[0]) ==  np.int64:
            date = [str(d) for d in self.df[name_date_column]]
            self.df[name_date_column] = pd.Series(np.array(date))
            print('type of 1 elmt: ',type(self.df.iloc[0,0]))
            self.df.to_csv(self.path_target, index=False)
            self.data_have_been_imported = True
            print('we copied with to csv')
        list_columns = self.name_columns
        format_date = self.textEdit_dateFormat.toPlainText()
        initDate = datetime.datetime.strptime(self.df[name_date_column].iloc[0],format_date).strftime(format="%Y-%m-%d %H:%M")
        endDate = datetime.datetime.strptime(self.df[name_date_column].iloc[-1],format_date).strftime(format="%Y-%m-%d %H:%M")
        ...
        self.dic_imported_data = {'type_data':type_data, 'initDate':initDate, 'endDate':endDate, 'name_date_column':name_date_column, 'name_time_column':name_time_column,
                                    'list_columns':list_columns, 'format_date':format_date, 'type_asset':type_asset, 'broker':broker, 'description':description}
